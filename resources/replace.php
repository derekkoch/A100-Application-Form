<?php
	...
	//$sqlDup = "SELECT * FROM applications INNER JOIN identity ON applications.identity_id = identity.identity_id 
		...
		$sqlDup = "SELECT * FROM applications, applicants, users WHERE applications.applicant_id = applicnt.applicant_id 
		AND users.user_id = applicants_user_id AND users.email = '$emailCheck'";
	...
	if($dupCount >1){
		//echo "Someone has enrolled in: " . $_POST['cohort_name'] . " with the provided email address already, thank you for your interest.";
		  echo "Someone has enrolled with the address: " . $_POST['email'] . " please continue your application. or contact us for more info.";
	}else{

	 	//$reqArray = mysqli_query($formCon, "SELECT field_name, is_required FROM fields");
	 	$reqArray = mysqli_query($appCon, "SELECT field_name, is_required FROM fields");
	 	$malformedInput = 0;
		...
		$tableInfo = array('applicants', 'users', 'referrals','schedules', 'experiences', 'materials');
